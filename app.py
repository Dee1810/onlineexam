...
def displayresults():
    if 'loggedin' in session:
        ...
        #student_id=cursor.execute("SELECT student_id FROM STUDENT WHERE student_email=%s",(student_email,))
...
def take_exam(examid):
    ...
    if request.method=='GET':
        ...
        #if request.method=='POST':
        #examid=request.form.get("exam__id",None)
        #print("exam id is")
        print(examid[1:-1])
        examid=examid[1:-1]
        session['examid']=examid
